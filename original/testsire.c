...
FILE *finput,*foutput,*foutput1,*finput1,*femittances,*fdist,*fdistout;
...
char *distribnamout="_DISTRIB_OUT_";
...
char path[]="RES";
char ext[]=".txt";
char *grname,*emitname,*distname,*twissname,*distnameout;    // Files produced
...
//BEGIN READING TWISS PARAMETERS FROM MADX FILE AND INPUT PARAMETERS FROM INPUT FILE
  ...
  {      
    ...
    distnameout=(char *)malloc(strlen(path)+strlen(dummyext)+strlen(distribnamout)+10);
    strcpy(distnameout,path);
    strcat(distnameout,distribnamout);
    strcat(distnameout,dummyext);
  }
	...
	if(KINJ==NINJ)
	  {
		...
		for(cont=0; cont<numpart; cont++)
		{
		  fprintf(fdistout,"%.9e, %.9e, %.9e, %.9e, %.9e, %.9e\n",ex1[cont],ez1[cont],es1[cont],phix1[cont],phiz1[cont],phis1[cont]);
		}
    fclose(fdistout);
	      }
      else
